...
conv_nxs = [125, 125*3, 125*3**2, 125*3**3, 125*3**4]
...
for n in conv_nxs:
    ...
    # plt.plot(x, u)
    # plt.plot(x,initial_condition_adv(x))
    er = np.linalg.norm(u - initial_condition_adv(x), ord = np.inf)
    errb.append(er)
    
        

plt.figure(figsize=(8, 6))
plt.loglog(hx, errb, "-og", label="weno3_tvdrk3")
